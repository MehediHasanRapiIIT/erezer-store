 ...
 *
 * Angular inlines constants into the bundle at build time, so any URL written
 * in source pins one image to one environment. Instead, the container writes
 * `/env.js` on startup from its own environment variables and every
