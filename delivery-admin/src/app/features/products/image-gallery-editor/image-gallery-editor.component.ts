    ...
    <section class="bg-white rounded-xl border border-gray-200 p-5">
      ...
      @if (!canList()) {
        <p class="rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-500">
          Showing images needs the “See products” or “Manage product photos” permission.
        </p>
      } @else if (loading()) {
              ...
              <div class="space-y-2 p-3">
                <div>
                  ...
                  <input
                    ...
                    (blur)="saveAlt(img)"
                    [disabled]="!perms.can('products.images')"
                    placeholder="Description for screen readers"
                    class="mt-1 w-full rounded-lg border border-gray-200 px-2 py-1.5 text-xs disabled:bg-gray-50 disabled:text-gray-500"
                  />
                </div>
                <div class="flex items-center justify-between">
                  <label class="flex items-center gap-2 text-xs font-medium text-gray-500">
                    ...
                    <input
                      ...
                      (blur)="saveSort(img)"
                      [disabled]="!perms.can('products.images')"
                      class="w-16 rounded-lg border border-gray-200 px-2 py-1 text-xs disabled:bg-gray-50 disabled:text-gray-500"
                    />
                  </label>
                  @if (!img.isPrimary && perms.can('products.images')) {
...
export class ImageGalleryEditorComponent implements OnChanges {
  ...
  protected saveAlt(img: ProductImageResponse): void {
    if (!this.perms.can('products.images')) return;
