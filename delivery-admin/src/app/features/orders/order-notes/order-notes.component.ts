...
@Component({
  ...
  template: `
    @if (perms.canAny('orders.notes.view', 'orders.notes.add')) {
    <section class="bg-white rounded-xl border border-gray-200 p-5">
      ...
      @if (!perms.can('orders.notes.view')) {
        <p class="text-xs text-gray-400">Notes are hidden. Reading them needs the “Read order notes” permission.</p>
      } @else if (loading()) {
          ...
          @for (n of notes(); track n.id) {
            <li class="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
              <div class="flex items-start justify-between gap-3">
                ...
                @if (perms.can('orders.notes.delete')) {
...
export class OrderNotesComponent implements OnChanges {
  ...
  reload(): void {
    ...
    if (!id || !this.perms.can('orders.notes.view')) return;
